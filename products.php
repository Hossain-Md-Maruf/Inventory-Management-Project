<?php
    ...
    if(isset($_POST['purchase'])){
        $purchased = "update shooppinginfo set status='Purchased' where userId='$id' and status='Selected'";
        if($conn->query($purchased)===true)
                {
            ...
            <div class="leftcolumn">
                ...
                <div class="card">
                    <div class="text-center">
                        <?php
                        if($thisUser['is_admin']==1 || $thisUser['is_admin']==2) {
                            ?>
                    ...
                    <div class="table_container">
                        ...
                        <form method="POST">
                                ...
                                <tbody>
                                    <?php
                                        if(mysqli_num_rows($res)>0){
                                            ...
                                            {
                                                $product_id = $row['id'];
                                                $find= "SELECT * FROM shooppinginfo WHERE userID='$id' and productID='$product_id' and status='Selected'";
                                                $found= mysqli_fetch_assoc($conn->query($find));
                                                



                                                echo "<td><a href='viewShoppingDetails.php?id=".$row['id']."' class='btn btn-success btn-sm'>".
                                                    "<span class='glyphicon glyphicon-eye-open'></span> </a>";
                                                echo "<a href='buyProduct.php?id=".$row['id']."' class='btn btn-warning btn-sm'>".
                                                    "<span class='glyphicon glyphicon-shopping-cart'></span> </a>";
                                                    if($found['status']=='Selected')
                                                    {
                                                    echo "<a href='cancelSelection.php?id=" . $row['id'] . "' class='btn btn-danger btn-sm'>" .
                                                        "<span class='glyphicon glyphicon-trash'></span> </a></td>";
                                                    }
                            ...
                            if($thisUser['is_admin']==0) {
                                ?>
                            <div class="form-group" style="text-align: center;">
                                                <button type="submit" value="purchase" name="purchase" class="btn btn-success">Purchase</button>
                                            </div>
                                            <?php
                            }
                            ?>
